...
int ADCs[32];         //storing values from ADCS
float Voltages[32];   //storing values from ADCS converted to voltage
                      //arduino ADC returns values in integer from 0 to 1023
float Temperatures[32];//storing values from ADCS converted to temperature in degrees Celsius
...
void loop() {
  ...
  for(int i = 0b0; i<0b10; i++)
   {  
      ...
      //digitalWrite(MUX_S3, i&0b1000); //3nd bit of i controls S2
